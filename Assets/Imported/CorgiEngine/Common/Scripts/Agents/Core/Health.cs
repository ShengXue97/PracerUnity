...
namespace MoreMountains.CorgiEngine
{
	...
	public class Health : MonoBehaviour
	{
		...
		public virtual void Damage(int damage,GameObject instigator, float flickerDuration, float invincibilityDuration)
		{
			...
			if ((CurrentHealth <= 0) && (InitialHealth != 0) && (CurrentHealth<MaximumHealth))
			{
				return;
			}
			...
			if (CurrentHealth <= 0)
			{
				...
				if (SceneManager.GetActiveScene () == SceneManager.GetSceneByName ("Level1")) {
					if (GetComponent<Controls> () != null) {
						if (Camera.main.GetComponent<CameraFollow> ().endgame == 0 && GetComponent<Controls> ().GameMode != "Deathmatch" && GetComponent<Controls> ().photonView.isMine) {
