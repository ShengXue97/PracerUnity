...
{		
	...
	public class ProjectileWeapon : Weapon 
	{
		...
		protected override void WeaponUse()
		{
			...
			Camera.main.GetComponent<CameraFollow> ().uses -= 1;
			if (Camera.main.GetComponent<CameraFollow> ().uses <= 0) {
				Camera.main.GetComponent<CameraFollow> ().UsedUpWeapon = true;
			}
		}
		...
		public virtual GameObject SpawnProjectile(Vector3 spawnPosition,bool triggerObjectActivation=true)
		{
			...
			//we do not throw the object if player is holding down
			if (Owner.gameObject.GetComponent<CharacterCrouch> ().IsCrouchingOrCrawling ()) {
				if (nextGameObject.GetComponent<ThrownObject> () != null) {
					nextGameObject.GetComponent<ThrownObject> ().IsThrown=false;
				}
			}
