...
{	
	...
	public class CharacterLadder : CharacterAbility 
	{
		...
		protected virtual void SetClimbingState()
		{
			...
			if (GetComponent<Controls> ().CurrentGravity != 0f) {
				LastGravity = GetComponent<Controls> ().CurrentGravity;
				GetComponent<Controls> ().CurrentGravity = 0f;
			}
		}
		...
		protected virtual void GetOffTheLadder()
		{
			...
			GetComponent<Controls> ().CurrentGravity = -30f;
			_controller.CollisionsOn ();
