...
{	
	...
	public class CharacterJetpack : CharacterAbility 
	{
		...
		public float JetpackFuelDuration;
		...
		protected Vector3 _initialPosition2;
		...
		public ParticleSystem.EmissionModule emissionModule2;
		/// <summary>
		...
		{
			base.Initialization();
			JetpackFuelDuration = 10f;
			JetpackForce = 7.5f;
			if (ParticleEmitter!=null)
			{
				_initialPosition = ParticleEmitter.transform.localPosition;
				_initialPosition2 = ParticleEmitter2.transform.localPosition;
				emissionModule = ParticleEmitter.emission;
				emissionModule2 = ParticleEmitter2.emission;
				GetComponent<PhotonView> ().RPC ("ClientSpawn", PhotonTargets.All, false);
			}
		...
		public virtual void JetpackStart()
		{
			...
			if ((!JetpackUnlimited) && (JetpackFuelDurationLeft <= 0f)) 
			{
				...
				GetComponent<Controls>().ChangeItem ("none");
				return;
			}
			...
			if (ParticleEmitter!=null)
			{
				...
				ParticleSystem.EmissionModule emissionModule2 = ParticleEmitter2.emission;
				GetComponent<PhotonView> ().RPC ("ClientSpawn", PhotonTargets.All, true);
			}
		...
		public override void Flip()
		{
			...
			{
				// we invert the rotation of the particle emitter
				ParticleEmitter2.transform.eulerAngles = new Vector3 (ParticleEmitter2.transform.eulerAngles.x, ParticleEmitter2.transform.eulerAngles.y + 180, ParticleEmitter2.transform.eulerAngles.z);	

				// we mirror its position around the transform's center
				if (ParticleEmitter2.transform.localPosition == _initialPosition2)
				{
					ParticleEmitter2.transform.localPosition = Vector3.Scale (_initialPosition2, _character.FlipValue);	
				} 
				else 
				{
					ParticleEmitter2.transform.localPosition = _initialPosition2;	
				}
			}
		}
		...
		[PunRPC]
		public void ClientSpawn(bool active)
		{
			emissionModule.enabled = active;
			emissionModule2.enabled = active;
		}
