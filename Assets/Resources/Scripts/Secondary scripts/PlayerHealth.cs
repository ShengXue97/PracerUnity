...
namespace MoreMountains.CorgiEngine
{
	...
	public class PlayerHealth : MonoBehaviour
	{
		...
		public virtual void Damage(int damage,GameObject instigator, float flickerDuration, float invincibilityDuration)
		{
			...
			//If damage is negaative and we are at max health, exit
			if (CurrentHealth >= MaximumHealth && damage <= 0) {
				return;
			}

			GetComponent<PhotonView> ().photonView.RPC ("DamagePlayer", PhotonTargets.All, damage, flickerDuration, invincibilityDuration);

		}

		[PunRPC]
		public void DamagePlayer(int damage, float flickerDuration, float invincibilityDuration)
		{
		...
		public virtual void Kill()
		{
			...
			if (_character != null)
			{
				...
				//_character.Reset ();
				if (GetComponent<PhotonView> ().photonView.isMine  && SceneManager.GetActiveScene () == SceneManager.GetSceneByName ("Level1")) {
					if (Camera.main.GetComponent<CameraFollow> ().endgame == 0) {
